class SpotifyPlayer {
  constructor(options = {}) {
    ...
    this.exchangeHost = options.exchangeHost || 'https://playing-spotify-server.onrender.com';
  ...
  togglePlayback() {
    return this.fetchPlayer().then(response => {
      return fetch(response.is_playing ? 'https://api.spotify.com/v1/me/player/pause' : 'https://api.spotify.com/v1/me/player/play', {
        method: "PUT",
        headers: { 'Authorization': 'Bearer ' + this.accessToken }
      }).then(response => {
        return response.json()
        ...
        if (json.error.status != 200) {
          console.log(json.error.message + " - " + json.error.reason);
        }
        else {
          return Promise.resolve(json);
        }
